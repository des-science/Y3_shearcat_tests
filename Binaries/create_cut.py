...
#master = h5.File('/global/cscratch1/sd/troxel/cats_des_y3/Y3_mastercat_12_3_19.h5', 'r')
#master = h5.File('/project/projectdirs/des/www/y3_cats//Y3_mastercat_12_3_19.h5', 'r')
master = h5.File('/global/cscratch1/sd/troxel/cats_des_y3/Y3_mastercat_{}.h5'.format(mastercat_version), 'r')

stellar_loci = Table.read('data/covey_medianlocus.tbl', format='cds')
...
gold_cut = make_gold_cuts(master)
shape_cut = make_shape_cuts(master)
...
shape_select_bool = gold_cut*shape_cut

print('N_shape = {0}'.format(np.sum(shape_select_bool)))

highe_shape_cut = shape_select_bool*highe_cut

print('N_highe_shape = {0}'.format(np.sum(highe_shape_cut)))
...
output_cat.write('./data/binary_cut_{0}.fits'.format(mastercat_version))
