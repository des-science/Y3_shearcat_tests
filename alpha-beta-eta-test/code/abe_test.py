...
def parse_args():
    ...
    parser.add_argument('--taus',
                        ...
                        default=['/home/dfa/sobreira/alsina/Y3_shearcat_tests/alpha-beta-eta-test/measured_correlations/TAUS_FLASK.fits',
                                 ...
                                 '/home/dfa/sobreira/alsina/Y3_shearcat_tests/alpha-beta-eta-test/measured_correlations/TAUS_FLASK.fits',
                                 '/home/dfa/sobreira/alsina/Y3_shearcat_tests/alpha-beta-eta-test/measured_correlations/TAUS_FLASK.fits'],
                        help='List of fits file containing all tau stats used to estimate abe')
...
def getxibias(samples, datarhos,  nsig=1, plots=False, nameterms='terms_dxi.png',dxiname='dxi.png'):
...
def getxibias_tomo(i, j, samples_list, datarhos,  nsig=1, plots=False, nameterms='terms_dxi.png',dxiname='dxi.png'):
    from src.readfits import read_rhos
    from src.maxlikelihood import bestparameters
    from src.plot_stats import pretty_rho
    from astropy.io import fits
...
def getmodelflagsandnames(args):
    ...
    if(args.abe):
        ...
        namemc = 'mcmc_alpha-beta-eta_eq_' + str(args.eq) + '_.png'
        namecont = 'contours_alpha-beta-eta_eq_' + str(args.eq) + '_.png'
        nameterms = 'termsdxi_alpha-beta-eta_eq_' + str(args.eq) + '_.png'
        namecovmat = 'covmatrix_alpha-beta-eta_eq_' + str(args.eq) + '_.png'
        namedxip = 'xibias_abe_' + str(args.eq) + '_.png'
        filename =  'abe_dxi.fits'            
    ## ALPHA-BETA
    if(args.ab):
        print("### Runing alpha and beta test ### ")
        mflags = [True, True, False] ##alpha,beta,eta
        namemc = 'mcmc_alpha-beta_eq_' + str(args.eq) + '_.png'
        namecont = 'contours_alpha-beta_eq_' + str(args.eq) + '_.png'
        nameterms = 'termsdxi_alpha-beta_eq_' + str(args.eq) + '_.png'
        namecovmat = 'covmatrix_alpha-beta_eq_' + str(args.eq) + '_.png'
        namedxip = 'xibias_ab_' + str(args.eq) + '_.png'
        filename =  'ab_dxi.fits'
    ## ALPHA-ETA
    if(args.ae):
        print("### Runing alpha and eta test ### ")
        mflags = [True, False, True] ##alpha,eta,eta
        namemc = 'mcmc_alpha-eta_eq_' + str(args.eq) + '_.png'
        namecont = 'contours_alpha-eta_eq_' + str(args.eq) + '_.png'
        nameterms = 'termsdxi_alpha-eta_eq_' + str(args.eq) + '_.png'
        namecovmat = 'covmatrix_alpha-eta_eq_' + str(args.eq) + '_.png'
        namedxip = 'xibias_ae_' + str(args.eq) + '_.png'
        filename =  'ae_dxi.fits'
    ## BETA-ETA
    if(args.be):
        print("### Runing beta and eta test ### ")
        mflags = [True, False, True] ##beta,eta,eta
        namemc = 'mcmc_beta-eta_eq_' + str(args.eq) + '_.png'
        namecont = 'contours_beta-eta_eq_' + str(args.eq) + '_.png'
        nameterms = 'termsdxi_beta-eta_eq_' + str(args.eq) + '_.png'
        namecovmat = 'covmatrix_beta-eta_eq_' + str(args.eq) + '_.png'
        namedxip = 'xibias_be_' + str(args.eq) + '_.png'
        filename =  'be_dxi.fits' 
    ## ALPHA
    if(args.a):
        print("### Runing alpha test ### ")
        mflags = [True, False, False] ##alpha,beta,eta
        namemc = 'mcmc_alpha_eq_' + str(args.eq) + '_.png'
        namecont = 'contours_alpha_eq_' + str(args.eq) + '_.png'
        nameterms = 'termsdxi_alpha_eq_' + str(args.eq) + '_.png'
        namecovmat = 'covmatrix_alpha_eq_' + str(args.eq) + '_.png'
        namedxip = 'xibias_a_' + str(args.eq) + '_.png'
        filename =  'a_dxi.fits'
    ## Beta
    if(args.b):
        print("### Runing beta test ### ")
        mflags = [False, True, False] ##alpha,beta,eta
        namemc = 'mcmc_beta_eq_' + str(args.eq) + '_.png'
        namecont = 'contours_beta_eq_' + str(args.eq) + '_.png'
        nameterms = 'termsdxi_beta_eq_' + str(args.eq) + '_.png'
        namecovmat = 'covmatrix_beta_eq_' + str(args.eq) + '_.png'
        namedxip = 'xibias_b_' + str(args.eq) + '_.png'
        filename =  'b_dxi.fits'
    ## Eta
    if(args.e):
        print("### Runing eta test ### ")
        mflags = [False, False, True] ##alpha,eta,eta
        namemc = 'mcmc_eta_eq_' + str(args.eq) + '_.png'
        namecont = 'contours_eta_eq_' + str(args.eq) + '_.png'
        nameterms = 'termsdxi_eta_eq_' + str(args.eq) + '_.png'
        namecovmat = 'covmatrix_eta_eq_' + str(args.eq) + '_.png'
        namedxip = 'xibias_e_' + str(args.eq) + '_.png'
        filename =  'e_dxi.fits'
    return [mflags, namemc, namecont, nameterms, namecovmat, namedxip, filename]
...
def write_singlexip(args, samplesp, samplesm):
    from src.readfits import read_rhos
    mflags, namemc, namecont, nameterms, namecovmat, namedxip, filename = getmodelflagsandnames(args)
    ...
    meanr, xip, covxip = getxibias(samplesp, datarhosp,
                                   plots=args.plots,
                                   ...
                                   filename=outpath + 'p_' +
                                   filename )
    meanr, xim, covxim = getxibias(samplesm, datarhosm,
                                   plots=args.plots,
...
def write_tomoxip(args, samplesp_list, samplesm_list):
    from src.readfits import read_rhos
    mflags, namemc, namecont, nameterms, namecovmat, namedxip, filename = getmodelflagsandnames(args)
    meanr, rhos,  covrhos =  read_rhos(args.rhoscosmo)
    datarhosp =  [meanr, [rhos[2*i] for i in range(6)],  [ covrhos[2*i] for i in range(6)]  ]
    ...
    for i,j in bin_pairs:
        print(i, j)
        meanr, xip, covxip = getxibias_tomo(i, j, samplesp_list,
                                            datarhosp, plots=args.plots, nameterms=plotspath + 'p_' +
                                            nameterms, dxiname=plotspath +'p_' + namedxip,
...
def SINGLE_TAU(args, taufile,  outpath,  plotspath, zbin=''):
    import numpy as np
    ...
    meanr, taus,  covtaus =  read_taus(taufile, minscale=args.minscale, maxscale=args.maxscale)
    data = {}
    ...
    if args.samecontaminant:
        ...
        samplesp, chainsp =  samples, chains
        samplesm, chainsm =  samples, chains
    else:
    ...
    return samplesp, chainsp, samplesm, chainsm

def ALLTOMOTAUS(args, taufiles,  outpath,  plotspath):
    if (args.plots):
        xlim = [0.1, 300.]
        ...
        plotalltausfits(taufile, outpath=plotspath, xlim=xlim)
    ...
    if args.samecontaminant:
        ...
        print( ' mcmc parameters xi join',  'nsig=', nsig, ' percentiles: ',  mcmcpars)
        
        if(args.plots):
            plot_samplesdist(samples, chains, mflags, nwalkers, nsteps,  namemc, namecont )
            plotcovpars(samples, namecovmat=plotspath +  namecovmat)

        meanr, xip, covxip = getxibias(samples, datarhosp,
                                       plots=args.plots, nameterms=plotspath + 'p_' + nameterms,
                                       dxiname=plotspath +'p_' + namedxip, filename=outpath + 'p_' +
                                       filename )
        meanr, xim, covxim = getxibias(samples,
                                       datarhosm,plots=args.plots, nameterms=plotspath+'m_' +
                                       nameterms, dxiname=plotspath+'m_' + namedxip, filename=outpath
                                       +'m_' + filename )
    else:
        samplesp, chainsp = RUNtest(i_guess, data, nwalkers, nsteps,
                                    eq=eq, mflags=mflags, xip=True ,
        ...
        print( ' mcmc parameters xim',  'nsig=', nsig, ' percentiles: ',  mcmcpars)
...
def main():
    ...
    if args.singletau is not None:
        ...
        samplesp, chainsp, samplesm, chainsm =  SINGLE_TAU(args, taufile, outpath, plotspath)
        mcmcpars = percentiles(samplesp, nsig=1) 
        print( ' mcmc parameters xi+',  'nsig=', 1, ' percentiles: ',  mcmcpars)
        mcmcpars = percentiles(samplesm, nsig=1) 
        print( ' mcmc parameters xi-',  'nsig=', 1, ' percentiles: ',  mcmcpars)
        write_singlexip(args, samplesp, samplesm)

    else:
        samplesp_list = []
        samplesm_list = []
        for i,  taufile in enumerate(taufiles):
            samplesp, chainsp, samplesm, chainsm =  SINGLE_TAU(args, taufile, outpath, plotspath, zbin=str(i + 1))
            mcmcpars = percentiles(samplesp, nsig=1) 
            print( ' mcmc parameters xi+',  'nsig=', 1, 'zbin=', i + 1, ' percentiles: ',  mcmcpars)
            mcmcpars = percentiles(samplesm, nsig=1) 
            print( ' mcmc parameters xi-',  'nsig=', 1, 'zbin=', i + 1, ' percentiles: ',  mcmcpars)
            samplesp_list.append(samplesp)
            samplesm_list.append(samplesm)
        write_tomoxip(args, samplesp_list, samplesm_list)
