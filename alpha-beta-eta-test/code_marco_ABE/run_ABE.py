...
class Jack_MAP(object):
    def __init__(self, conf, distance_file, ra_a, dec_a, ra_b, dec_b, hpix_a, hpix_b, w_a,w_b, e1_a, e2_a, e1_b, e2_b, k = None,  corr='GG',  number_of_cores=2, fact_dist=2,
                 ...
                 verbose=False, overwrite=False, cov = False,outp_fold='',label ='',init=0):
        ...
        self.label = label
        try:
            conf['nbins']=conf['nbins ']
        except:
            pass
        conf['nodes']=number_of_cores
        print (conf)
        self.conf = conf
...
"""
waaaarning!!!! I changed the weights to 1.
...
if __name__ == '__main__':
    ...
    for i in range(len(runsss)):
        ...
        THIS DOES NOT SUBTRACT THE MEAN.
        if  ((rewrite) or (not os.path.exists(mute_dict['path_pkl']+"_noweight_nomean2"+".pkl"))):
            list_run.append(i)
    '''   
    ...
    while run_count<len(list_run):
        ...
        run_MAP_one(list_run[len(list_run)-(run_count+comm.rank)-1],
               list_run_init[len(list_run)-(run_count+comm.rank)-1])#,runs_path)
        run_count+=comm.size
