...
class Jack_MAP(object):
    ...
    def prolog(self):
        ...
        print ("max distance region computed")
        if self.corr == 'GG':
            gm = 3
...
def run_MAP(i):
    ...
    rewrite = False
    runs_path = load_obj("runs_path_ABE")
    mute_dict = load_obj(runs_path[i])
    print ("loaded ")
    if  ((rewrite) or (not os.path.exists(mute_dict['path_pkl']+"_noweight"+".pkl"))):
...
if __name__ == '__main__':
    ...
    while run_count<len(list_run):
        ...
        run_MAP(list_run[len(list_run)-(run_count+comm.rank)-1])#,runs_path)
        run_count+=comm.size
